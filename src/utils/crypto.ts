/**
 * NEON FORENSIC - Cryptographic Hashing & Verification Engine
 * Implements standard SHA-256, SHA-512 and MD5 (flagged non-integrity safe)
 */
...
export async function computeSha512(data: string | Uint8Array | ArrayBuffer): Promise<string> {
  const buffer = typeof data === "string" ? new TextEncoder().encode(data) : data;
...
/**
 * Lightweight MD5 implementation for legacy identification only.
 * Clearly marked as non-integrity-safe per forensic standards.
 */
export function computeLegacyMd5(message: string): string {
  ...
  words[((str.length * 8) >> 5)] |= 0x80 << ((str.length * 8) % 32);
  words[(((str.length * 8 + 64) >>> 9) << 4) + 14] = str.length * 8;
...
/**
 * Verifies chain of custody block integrity
 */
export async function verifyAuditChain(
  ...
  for (let i = 0; i < records.length; i++) {
    ...
    const expectedPrev = i === 0 ? "GENESIS_ROOT_HASH_000000000000000000000000000000000000000000000000" : records[i - 1].currentEventHash;
    if (record.previousEventHash !== expectedPrev) {
      return { isValid: false, brokenAtIndex: i };
    }

    const payload = `${record.previousEventHash}|${record.timestamp}|${record.investigator}|${record.action}|${record.evidenceId}`;
    const calculatedHash = await computeSha256(payload);
    if (calculatedHash !== record.currentEventHash) {
      return { isValid: false, brokenAtIndex: i };
    }
  }
  return { isValid: true };
